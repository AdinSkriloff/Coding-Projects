...
public class SnackMachineTest {
    ...
    public void testTotalProfit() {
        final int ONE_TWO_SLOT_MACHINE_COST = -2000;
        assertEquals(ONE_TWO_SLOT_MACHINE_COST, VendingMachine.getTotalProfit());
        SnackMachine sMachine1 = new SnackMachine(pList);
        int change = sMachine.buy(0, 0, 2);
        assertEquals(75, change);
        change = sMachine.buy(0, 0, 1);
        assertEquals(-1, change);
        assertEquals(ONE_TWO_SLOT_MACHINE_COST - 1875, VendingMachine.getTotalProfit());
    }
